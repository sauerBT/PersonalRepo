function [tout, yout] = RK4_CSTR_S_3(tspan,y0,ssize, m0, m1, M, x1_in, x2_in, x3_in)
...
while (k < N)
  ...
  k = k + 1;
 % RK4 Slope Calc 
  s1 =  CSTR_S_3(t, y, m0, m1, M, x1_in, x2_in, x3_in); s1 = s1(:);
  s2 =  CSTR_S_3(t + h/2, y + h*s1/2, m0, m1, M, x1_in, x2_in, x3_in); s2=s2(:);
  s3 = CSTR_S_3(t + h/2, y + h*s2/2, m0, m1, M, x1_in, x2_in, x3_in); s3=s3(:);
  s4 = CSTR_S_3(t + h, y + h*s3, m0, m1, M, x1_in, x2_in, x3_in); s4=s4(:);
